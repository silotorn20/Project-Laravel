    ...
    <div class="container">
        ...
        <div class="container mt-5">
            ...
            <div class="card" style="border-color: blue; padding: 5px;">
                <div class="card-body">
                    <form id="rating-form" method="POST" action="{{ route('reviews.storeMybbok',['id_book' => $id_book]) }}"
                        class="mt-3">
                        @csrf
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var currentPage = 1;
            var totalPages = 0;
            var pagesToDisplay = 1; // จำนวนหน้าที่จะแสดง

            function renderPage(pageNum) {
                pdfjsLib.getDocument(url).promise.then(function(pdf) {
                    var endPage = Math.min(pageNum + pagesToDisplay - 1, totalPages);
            ...
            function goToPage(pageNum) {
                if (pageNum >= 1 && pageNum <= totalPages - pagesToDisplay + 1) {
                    currentPage = pageNum;
            ...
            document.getElementById('nextPage').addEventListener('click', function() {
                if (currentPage + pagesToDisplay <= totalPages) {
                    goToPage(currentPage + 1);
                }
            });
        });
